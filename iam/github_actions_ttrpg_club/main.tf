locals {
  lambda_function_arn       = "arn:aws:lambda:${var.region}:${data.aws_caller_identity.current.account_id}:function:${data.terraform_remote_state.lambda_api.outputs.lambda_function_name}"
  frontend_bucket_arn       = "arn:aws:s3:::${data.terraform_remote_state.frontend.outputs.bucket_name}"
  frontend_distribution_arn = "arn:aws:cloudfront::${data.aws_caller_identity.current.account_id}:distribution/${data.terraform_remote_state.frontend.outputs.distribution_id}"
}
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = local.frontend_distribution_arn
      }
    ]
  })
}
