...
locals {
  ...
  route_keys = toset([
    ...
    "POST /signup",
    "POST /auth/telegram",
    # No POST /auth/dev-login here — unlike ../ttrpg_club_api_dev, prod has no
    # dev_login_secret variable at all, so this route doesn't exist at the API
    # Gateway level, not just at the Lambda's own routing level.
    "GET /game-systems",
