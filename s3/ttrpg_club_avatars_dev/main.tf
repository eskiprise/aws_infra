# Dev counterpart to ../ttrpg_club_avatars_prod — same shape, separate bucket/distribution.
# Replaces the old unsuffixed ../ttrpg_club_avatars module (retire that one via
# `terraform destroy` once this is applied and cut over — see aws_infra/README.md).

resource "aws_s3_bucket" "avatars" {
  bucket = "ttrpg-club-avatars-dev"
...
resource "aws_cloudfront_distribution" "avatars" {
  ...
  comment     = "TTRPG club — member/GM profile pictures (public read, dev)"
